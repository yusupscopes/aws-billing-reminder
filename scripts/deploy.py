...
def deploy(environment, region, profile):
    """Deploy the AWS Billing Reminder to AWS"""
    
    # Initialize AWS clients
    session = boto3.Session(profile_name=profile, region_name=region)
    s3 = session.client('s3')
    cfn = session.client('cloudformation')
    bucket_name = f'aws-billing-reminder-deployment-{environment}'
    stack_name = f'aws-billing-reminder-{environment}'
    s3_key = f'{stack_name}/function.zip'
    email_address = 'yusupmaulana950@gmail.com'
